...
static void config_write_bsc_single(struct vty *vty, struct bsc_config *bsc)
{
	...
	vty_out(vty, "  bts-jitter-buffer %s%s",
		bsc->bts_dejitter ? "on" : "off", VTY_NEWLINE);
	if (bsc->bts_jitter_delay_min)
		vty_out(vty, "  bts-jitter-delay-min %"PRIu32"%s", bsc->bts_jitter_delay_min, VTY_NEWLINE);
	if (bsc->bts_jitter_delay_max)
...
DEFUN(cfg_bsc_bts_dejitter,
      cfg_bsc_bts_dejitter_cmd,
      "bts-jitter-buffer (on|off)",
      DEJITTER_STR "\n" "Enable "DEJITTER_STR"\n" "Disable "DEJITTER_STR"\n")
{
	struct bsc_config *conf = vty->index;
	if (strcmp(argv[0], "on") == 0)
		conf->bts_dejitter = 1;
...
int bsc_nat_vty_init(struct bsc_nat *nat)
{
	...
	install_element(MGCP_NODE, &cfg_bsc_bts_dejitter_cmd);
	install_element(MGCP_NODE, &cfg_bsc_bts_jitter_delay_min_cmd);
	install_element(MGCP_NODE, &cfg_bsc_bts_jitter_delay_max_cmd);
